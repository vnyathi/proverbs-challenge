...
const css = `
  ...
  .pv*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
  .pv-root{
  ...
  /* Reader */
  .pv-rnav{display:flex;align-items:center;gap:8px;margin-bottom:10px;}
  .pv-rnavbtn{background:var(--card2);border:1px solid var(--line);border-radius:6px;padding:6px 10px;font-family:'Fraunces',serif;font-size:12px;color:var(--gold-d);cursor:pointer;}
  .pv-rnavbtn:disabled{opacity:.4;cursor:default;}
  ...
  .pv-vt{font-size:15px;line-height:1.6;color:var(--ink);}
  .pv-vhint{font-size:12px;color:var(--soft);font-style:italic;text-align:center;margin:8px 0;}
  .pv-selvbanner{background:#efe4c8;border:1px solid var(--gold);border-radius:8px;padding:10px 12px;margin:8px 0;}
  .pv-selvlbl{font-family:'Fraunces',serif;font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:var(--gold-d);margin-bottom:4px;}
  .pv-selvtxt{font-style:italic;font-size:14px;line-height:1.5;}
  .pv-selvref{font-family:'Fraunces',serif;font-size:11px;color:var(--soft);margin-top:3px;}
  ...
  .pv-loading{text-align:center;padding:30px;font-style:italic;color:var(--soft);}
`;
    ...
    <div>
      ...
      {picked && (
        <div className="pv-selvbanner">
          ...
          <button className="pv-usebtn" onClick={() => { onUseVerse(picked.text); setPicked(null); }}>
            Use as my favourite verse ✓
          </button>
        </div>
      )}
...
export default function ProverbsChallenge() {
  ...
  const [activeTab, setActiveTab] = useState({}); // chapter -> "read"|"journal"
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (r && r.value) {
          const parsed = JSON.parse(r.value); setMe(parsed);
          try { const mine = await store.get("user:"+parsed.id, true); if (mine && mine.value) setMyEntries(JSON.parse(mine.value).entries||{}); } catch (e) {}
        }
      } catch (e) {}
    ...
    <div className="pv-root">
      ...
      <div className="pv-wrap">
        <div className="pv-hero">
          ...
          <p className="pv-sub">Read, listen, and reflect on one chapter a day with friends.</p>
          ...
          <div className="pv-steps">
            ...
            <div className="pv-step"><div className="pv-snum">3</div><div className="pv-stext"><b>Do it with friends.</b> See each other's reflections and encourage one another.</div></div>
          </div>
